...
static void layers1(J d,K x) {
 std::cerr << "depth: " << d << ", " << kstring(x) << "\n";
 S s,nm=nullptr;
 if(x->t == -KS) {
  s=x->s;
 } else if(x->t == KS) {
  TORCH_CHECK(x->n>0, "empty symbol list");
  s=kS(x)[0];
  if(x->n>1) nm=kS(x)[1];
 } else if(!x->t) {
  TORCH_CHECK(x->n>0, "empty list");
  TORCH_CHECK(kK(x)[0]->t==-KS, "no module symbol: ",kstring(x));
  s=kK(x)[0]->s;
  if(x->n>1 && kK(x)[1]->t==-KS) nm=kK(x)[1]->s;
 } else {
  AT_ERROR("unrecognized arg(s): ", kstring(x));
 }
 Cast c=msym(s);
 if(container(c)) {
  auto a=parent(c);
 } else {
  auto a=anymodule(x,nm ? 2 : 1, c);
 }
}

static void layers2(J d,K x) {
 layers1(d,x->t || !x->n ? x : kK(x)[0]);
 if(!x->t) for(size_t i=1;i<x->n;i++) layers2(d+1,kK(x)[i]);}

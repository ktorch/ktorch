...
// --------------------------------------------------------------------------
// select - add convenience module for tensor.select(dim,ind)
// --------------------------------------------------------------------------
...
class TORCH_API SelectImpl : public torch::nn::Cloneable<SelectImpl> {
 public:
 SelectImpl(const SelectOptions& o) : options(o) {reset();}
 void reset() override {}
 void pretty_print(std::ostream& s) const override {s << "Select(dim=" << options.dim() << ",ind=" << options.ind() << ")";}
 torch::Tensor forward(const torch::Tensor& x) {
...
// --------------------------------------------------------------------------
// indexselect - add convenience module for tensor.index_select(,dim,ind)
// --------------------------------------------------------------------------
struct TORCH_API IndexSelectOptions {
 IndexSelectOptions(int64_t d,int64_t i) : dim_(d),ind_(torch::full({},torch::Scalar(i))) {}
 IndexSelectOptions(int64_t d,torch::Tensor i) : dim_(d),ind_(i) {}
 TORCH_ARG(int64_t, dim);
...
class TORCH_API IndexSelectImpl : public torch::nn::Cloneable<IndexSelectImpl> {
 public:
 IndexSelectImpl(const IndexSelectOptions& o) : options(o) {reset();}

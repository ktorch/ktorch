...
enum class Setting:char {
 ...
 complexfirst, countpad,  cuda,         cudadevices, cudnn,         cudnnversion, dampening,   
 decay,        decoder,   decoderlayer, detach,      deterministic, dilate,       dim,         
 divisor,      dlayers,   dropout,      dtype,       elayers,       encoder,      encoderlayer,
 end,          eps,       eval,         fn,          freeze,        full,         gradtol,     
 groups,       heads,     hidden,       history,     ignore,        in,           in1,         
 in2,          ind,       indices,      init,        inplace,       iter,         k,           
 kdim,         keepdim,   kvbias,       kvzeros,     lambda,        lastoffset,   layernorm,   
 layers,       log,       lower,        lr,          lrdecay,       magma,        margin,      
 max,          maxnorm,   min,          mkl,         mode,          momentum,     nesterov,    
 norm,         openmp,    out,          outpad,      outsize,       p,            pad,         
 padindex,     padmode,   ratio,        reduce,      rescale,       rows,         scale,       
 search,       shape,     size,         slope,       sparse,        stackframe,   start,       
 stride,       swap,      threads,      threshold,   track,         train,        transpose,   
...
*/
 ...
 std::array<std::tuple<S,Setting>,13> cset = {{            // configuration settings
